...
class Middleware {
    ...
    constructor(app, config) {
        ...
        // 配置静态资源加载中间件
        this.staticHandler(middleware.staticPath, rootPath);
        // 配置服务端模板渲染引擎中间件
        this.viewHandler(middleware.viewPath, rootPath);
        // 配置session中间件
        this.sessionHandler(middleware.sessionConfig, rootPath);
        // 初始化路由配置
        this.routerHandler(middleware.routerPath, rootPath);
    };
	 ...
	 */
    routerHandler(routerPath, rootPath) {
        ...
        this.app.use(routerMiddleware(routerPath));
    };
	 ...
	 * @param  {String}      rootPath      [应用根目录]
	 */
    staticHandler(staticPath, rootPath) {
        // 如果设置为false则不使用此中间件处理，自己实现
        if (typeof staticPath === false) return;
        // 如果不设置则默认使用根目录的上一级目录下的static目录
        staticPath = staticPath || path.join(rootPath, '../static');
    ...
    bodyParserRouter(bodyParser, rootPath) {
        ...
        const bodyParserMiddleware = bodyParser || require('koa-bodyparser')();
